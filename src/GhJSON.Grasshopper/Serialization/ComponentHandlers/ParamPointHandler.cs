...
{
    public class ParamPointHandler : ComponentHandlerBase
    {
        public ParamPointHandler()
            : base(supportedTypes: new[] { typeof(Param_Point) })
        {
        }

        public override int Priority => 50;

        public override bool CanHandle(IGH_DocumentObject obj) => obj is Param_Point;
        ...
        {
            if (obj is not Param_Point param || state?.PersistentData == null)
                return;

            PersistentDataHandlerUtilities.ApplyPersistentDataToParam(param, state.PersistentData, ConvertToPoint);
        }

        private static GH_Point ConvertToPoint(JToken token)
        {
