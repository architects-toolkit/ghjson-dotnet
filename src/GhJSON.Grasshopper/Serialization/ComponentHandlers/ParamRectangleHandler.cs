...
{
    public class ParamRectangleHandler : ComponentHandlerBase
    {
        public ParamRectangleHandler()
            : base(supportedTypes: new[] { typeof(Param_Rectangle) })
        {
        }

        public override int Priority => 50;

        public override bool CanHandle(IGH_DocumentObject obj) => obj is Param_Rectangle;
        ...
        {
            if (obj is not Param_Rectangle param || state?.PersistentData == null)
                return;

            PersistentDataHandlerUtilities.ApplyPersistentDataToParam(param, state.PersistentData, ConvertToRectangle);
        }

        private static GH_Rectangle ConvertToRectangle(JToken token)
        {
