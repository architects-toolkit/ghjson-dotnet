...
{
    public class ParamPlaneHandler : ComponentHandlerBase
    {
        public ParamPlaneHandler()
            : base(supportedTypes: new[] { typeof(Param_Plane) })
        {
        }

        public override int Priority => 50;

        public override bool CanHandle(IGH_DocumentObject obj) => obj is Param_Plane;
        ...
        {
            if (obj is not Param_Plane param || state?.PersistentData == null)
                return;

            PersistentDataHandlerUtilities.ApplyPersistentDataToParam(param, state.PersistentData, ConvertToPlane);
        }

        private static GH_Plane ConvertToPlane(JToken token)
        {
