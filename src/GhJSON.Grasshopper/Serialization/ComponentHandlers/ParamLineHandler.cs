...
{
    public class ParamLineHandler : ComponentHandlerBase
    {
        public ParamLineHandler()
            : base(supportedTypes: new[] { typeof(Param_Line) })
        {
        }

        public override int Priority => 50;

        public override bool CanHandle(IGH_DocumentObject obj) => obj is Param_Line;
        ...
        {
            if (obj is not Param_Line param || state?.PersistentData == null)
                return;

            PersistentDataHandlerUtilities.ApplyPersistentDataToParam(param, state.PersistentData, ConvertToLine);
        }

        private static GH_Line ConvertToLine(JToken token)
        {
