...
{
    public class ParamArcHandler : ComponentHandlerBase
    {
        public ParamArcHandler()
            : base(supportedTypes: new[] { typeof(Param_Arc) })
        {
        }

        public override int Priority => 50;

        public override bool CanHandle(IGH_DocumentObject obj) => obj is Param_Arc;
        ...
        {
            if (obj is not Param_Arc param || state?.PersistentData == null)
                return;

            PersistentDataHandlerUtilities.ApplyPersistentDataToParam(param, state.PersistentData, ConvertToArc);
        }

        private static GH_Arc ConvertToArc(JToken token)
        {
