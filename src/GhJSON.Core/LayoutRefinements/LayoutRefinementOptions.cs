...
{
    /// <summary>
    /// Options for the measurement-aware refinement passes that run after the core
    ...
    /// </summary>
    /// <remarks>
    /// The options are host-independent: every measurement-based pass resolves node
    /// geometry through <see cref="NodeMetricsProvider"/>. Hosts supply the provider —
    /// GhJSON.Grasshopper composes one from canvas objects; headless callers can feed
    /// metrics computed by any other means. With no provider set the passes degrade to
    /// no-ops.
    /// </remarks>
    public class LayoutRefinementOptions
    {
        /// <summary>
        ...
        /// <summary>
        /// Optional metrics lookup resolving a layout key to measured node geometry
        /// (bounds size, node kind, port deltas). This is the single measurement path
        /// for every refinement pass; when unset, measurement-based passes skip.
        /// </summary>
        public Func<Guid, LayoutNodeMetrics?>? NodeMetricsProvider { get; set; }
