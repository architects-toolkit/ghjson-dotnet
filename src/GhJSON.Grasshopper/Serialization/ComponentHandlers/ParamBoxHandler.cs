...
{
    public class ParamBoxHandler : ComponentHandlerBase
    {
        public ParamBoxHandler()
            : base(supportedTypes: new[] { typeof(Param_Box) })
        {
        }

        public override int Priority => 50;

        public override bool CanHandle(IGH_DocumentObject obj) => obj is Param_Box;
        ...
        {
            if (obj is not Param_Box param || state?.PersistentData == null)
                return;

            PersistentDataHandlerUtilities.ApplyPersistentDataToParam(param, state.PersistentData, ConvertToBox);
        }

        private static GH_Box ConvertToBox(JToken token)
        {
